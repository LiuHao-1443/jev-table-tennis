...
- **拍高是标准的 112px，而且刻意不拿它当旋钮。** 复盘 232 次真实决策，JEV 的落点误差中位 47px，
  112px 的拍子只覆盖它 **53%** 的答案。加大拍子很诱人，但那是错的：加到全屏它必然 100% 回球，
  那时候这个数字里一点模型信息都没有，而且**永远不会失败**。拍子半高就是**容错=考卷**，
  `T` 键只作对照实验用，默认保持标准。
- **真正能提高回球率的是「让它心算的距离变短」**：它的误差 ≈ 16.8 + 0.421 ×（要在 y 上外推的距离），
  r = 0.57 —— 球几乎不纵向移动时误差 23px，要推 100~200px 时误差 69px。它做不了
  「|vy| × 飞行时间」这种心算。所以球的角度上限收到 **0.55 rad**（原来 0.92），
  球变平 ⇒ 心算距离变短 ⇒ 误差真的变小。**容错一动不动**，而且这个说法是可证伪的：
  下次真实对局的误差中位如果没降到 40px 附近，就是我这个改法失败。`B` 键可切。
...
- **一次决策只问模型一个问题**——拍心该在哪（`place`）、站哪等（`ready`）、发球弧度（`serve`）。
  决策日志那行里其它数字（耗时、token、成本、比分、机械限速）都是本地账目；日志原本还会打印
  `方向`/`发力` 两个来自 assisted 基线的死字段，它们对物理零影响。
- **速度与脑子是直接交换，而且是可测的**：中继默认 551 px/s，一板给 JEV 3 次决策、还剩 0.78s 余量；
  650 只剩 2 次，900 只剩 1 次（闭环修正就没了）。而且**决定胜负的「最后一问」最准**（25px，
  第一问 37px）——因为它问的时候球最近；球越快，最后一问被推得越远、误差越大。
  改：`python3 server.py --cap N`。
- **它的落点误差是「题目」决定的**：232 次真实决策里中位偏 47px。改标签（序数反而更差，67px）、
  改档数（9 档 69px）、改提前量，实测都动不了它。真正能动它的是**要心算多少步**——见
  [`FINDINGS.md`](FINDINGS.md) 里「让它心算的距离变短」那一节。
